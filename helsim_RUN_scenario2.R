...
seed <- as.numeric(args[7])				# seed for repetitions
...
source(paste0(dataInputPath, "helsim_JY_FUNC_JT_b.R"))
...
doRealization <- function(params)
{
	...
	simData <- setupSimpleSD(params)	# Set up simulation data for village all worm counts and free living worms are set to 0 below
	...
	nextChemoIndex <- which.min(chemoTimes)						 
	nextChemoTime <- chemoTimes[nextChemoIndex]					# Time at which individuals receive next chemotherapy 	

	nextStep <- min(nextOutTime, time+params$maxStep, nextAgeTime, nextEmigrationTime, nextImmigrationTime, nextChemoTime)	# Determine which event will take place next
	...
	while((time<params$maxTime))    
	{
		...
		if((simData$freeLiving<params$equiData$L_break) & is.na(breakTime))  
		{
			breakTime <- time
		...
		{ 
			...
			{ 
		  		simData <- doDeathWithTreatAgeGroups(params, simData, time) 
			...
			{
				simData <- doDeathWithTreatAgeGroups(params, simData, time)	# update age groups/deaths just before chemotherapy
				simData <- doChemoSimple(params, simData, time)			# function adapted so that only individuals with away==FALSE are treated
